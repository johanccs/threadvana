...
order: 20
title: "Async Interview Traps  -  the c2 Gotchas Reviewed"
...
interview:
  - q: "Name three common async/await interview mistakes."
    a: "1. async void outside an event handler  -  untracked, un-catchable, crashes the process. 2. .Result or .Wait() on a Task  -  blocks the thread, deadlocks in classic ASP.NET/WPF. 3. Forgetting CancellationToken  -  operations run forever with no way to stop them. Interviewers want to hear that you know the WHY, not just the rule."
  - q: "What is the difference between Task.Run, Task.Factory.StartNew, and new Thread?"
    a: "Task.Run  -  default options on the thread pool, the go-to for modern code. Task.Factory.StartNew  -  more options (LongRunning, custom scheduler), but risky because default parameters differ from Task.Run (e.g., TaskCreationOptions.DenyChildAttach vs None). new Thread  -  dedicated OS thread, expensive, rarely needed since .NET 4.0."
---
