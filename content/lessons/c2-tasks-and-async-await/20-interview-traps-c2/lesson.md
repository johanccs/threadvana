...
order: 20
title: "Async Interview Traps Ã¢â‚¬â€ the c2 Gotchas Reviewed"
...
interview:
  - q: "Name three common async/await interview mistakes."
    a: "1. async void outside an event handler Ã¢â‚¬â€ untracked, un-catchable, crashes the process. 2. .Result or .Wait() on a Task Ã¢â‚¬â€ blocks the thread, deadlocks in classic ASP.NET/WPF. 3. Forgetting CancellationToken Ã¢â‚¬â€ operations run forever with no way to stop them. Interviewers want to hear that you know the WHY, not just the rule."
  - q: "What is the difference between Task.Run, Task.Factory.StartNew, and new Thread?"
    a: "Task.Run Ã¢â€ â€™ default options on the thread pool, the go-to for modern code. Task.Factory.StartNew Ã¢â€ â€™ more options (LongRunning, custom scheduler), but risky because default parameters differ from Task.Run (e.g., TaskCreationOptions.DenyChildAttach vs None). new Thread Ã¢â€ â€™ dedicated OS thread, expensive, rarely needed since .NET 4.0."
---
