...
order: 4
title: Under the Hood Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  What async/await Really Does
difficulty: advanced
...
interview:
  ...
  - q: When does an async method run synchronously?
    a: The first part of the method runs synchronously on the calling thread until the first await that actually yields. If an awaitable is already complete (e.g. Task.FromResult), the method continues synchronously Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  no state machine pause, no thread switch. If you await an incomplete task, only THEN does the compiler-generated machinery kick in.
---

## What is it?

`async` and `await` look like magic Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  write normal-looking code and it "just becomes asynchronous." Behind the scenes the C# compiler builds a **state machine** for you. Every async method gets split into pieces at each `await`, and the compiler generates the machinery to pause, resume, and track where you are.
...
The buzzer number is the **state** Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  you know which drink is yours. The barista knows to wake YOU, not someone else. The buzzer itself is the machine that connects the waiter (the background I/O) back to you (the continuation).
...
public async Task<string> MakeCoffeeAsync()
{
    ...
    await BoilWaterAsync();        // Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã â  Ã Â  PAUSE here
    Console.WriteLine("2. Pouring coffee...");
    await PourCoffeeAsync();       // Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã â  Ã Â  PAUSE here
...
```
state: 0 Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ run first sync part Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ await BoilWaterAsync
   Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã â ¦Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â ¬Å¡Ã Â¬ done? Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ goto 1
   Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã Â Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â ¬Å¡Ã Â¬ not done Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ save state=0, hook MoveNext as callback, RETURN
state: 1 Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ await PourCoffeeAsync
   Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã â ¦Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â ¬Å¡Ã Â¬ done? Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ goto 2
   Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã Â Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â ¬Å¡Ã Â¬ not done Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ save state=1, hook MoveNext as callback, RETURN
state: 2 Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ set result "coffee" Ã Æ Ã Â¢Ã Â¢Ã¢â  Â¬Ã Â Ã Â¢Ã¢â  Â¬Ã¢â  Â¢ Task completes
```
...
You can actually SEE the state machine using a decompiler Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  open any async method in ILSpy or dotPeek and look at the generated `<MethodName>d__N` struct.
...
- **ASP.NET Core / Console**: No special context Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  continuations run on any available thread-pool thread.
...
// This continuation now runs on a pool thread Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  not the UI thread!
```
...
## See it work Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  step by step
...
(The demo below still runs Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  press **Run demo** to see it live.)

## Watch out

- `async void` has no Task. Exceptions crash the process. Only use for event handlers.
- If the awaitable is ALREADY complete, the state machine never pauses Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  fully synchronous, zero allocation.
...
- Each `await` is a potential pause Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  the method returns a Task to the caller.
...
- `ConfigureAwait(false)` skips context capture Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  essential in library code.
