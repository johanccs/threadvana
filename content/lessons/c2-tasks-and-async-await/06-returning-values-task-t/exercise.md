...
1. Starts THREE `Task<int>` computations in parallel using `Task.Run` — each
...
`ExpensiveCompute` is provided — it simulates a slow calculation by sleeping
and returns `n * n`. Your job is only the async orchestration.

## Hints
1. `Task.Run(() => ExpensiveCompute(10))` returns a `Task<int>` — store each in a variable.
