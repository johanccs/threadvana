...
order: 6
title: Returning Values  -  Task of T
difficulty: beginner
...
interview:
  - q: What is the difference between Task and Task<T>?
    a: Task represents work that finishes with no return value (void-equivalent). Task<T> wraps a return value  -  the T becomes available after the task completes. Both can be awaited, composed with WhenAll/WhenAny, and carry exceptions the same way. Think of Task<T> as a "future"  -  a box that will contain a T once the work is done.
