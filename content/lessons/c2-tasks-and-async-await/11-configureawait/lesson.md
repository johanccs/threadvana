...
order: 11
title: "ConfigureAwait Ã¢â‚¬â€ Who Needs the Context?"
...
interview:
  ...
  - q: "In ASP.NET Core, do you still need ConfigureAwait(false)?"
    a: "No Ã¢â‚¬â€ ASP.NET Core removed the SynchronizationContext, so continuations already run on any thread by default. ConfigureAwait(false) does nothing for correctness, but it is harmless. In legacy ASP.NET Framework, omitting it inside a library can deadlock the request thread if the caller uses .Result."
---
