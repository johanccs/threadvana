...
order: 1
title: Meet the Task Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  a Promise of a Future Result
difficulty: beginner
...
and you get a Task back instantly Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  not the result, a *promise* of the result.
...
your drink behind the counter. The buzzer is not coffee Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  it is a promise that
...
and the receipt can carry a result back Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  something a raw `Thread` cannot do.

## See it move

Press **Run demo**. Watch the pool worker's lane pick up the order while the
main lane keeps going Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  the receipt came back instantly. Then look at the
...
- You might call `Task.Run(...)` and forget `await`. Then you are holding the
  receipt, not the coffee Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  a `Task<int>`, not an `int`. The compiler warns
  about most of these; listen to it.
- You might grab the result with `.Result` or `.Wait()`. That parks your thread
  at the counter until the drink is done Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  the opposite of what Tasks are for,
  and a classic way to deadlock. Use `await`.
- You might think `Task.Run` creates a new thread. It does not Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  it borrows one
...
- `Task.Run` puts work on the thread pool Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  the modern replacement for `new Thread`.
- `await` collects the result without blocking a thread.
- A raw `Thread` cannot hand you back a value; a `Task<T>` can.
- Avoid `.Result` and `.Wait()` Ã Æ Ã Â¢Ã Â¢Ã¢â ¬Å¡Ã Â¬Ã Â¢Ã¢â  Â¬Ã Â  blocking wastes the worker and can deadlock.