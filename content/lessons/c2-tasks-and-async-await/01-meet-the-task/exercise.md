...
`Solution.FetchNumber()` is provided — it pretends to be a slow web call and
...
1. Run `FetchNumber()` on the thread pool with `Task.Run(...)`. This hands you
   a `Task<int>` — a receipt for the number, not the number itself.
2. `await` that task to collect the number it delivers.
3. Store the number in `Solution.Result`.

We check that `Result` ends up holding 42 — proof that the task really ran and
...
3. The signature `public static async Task RunAsync()` is already there — `async` is what allows `await` inside.