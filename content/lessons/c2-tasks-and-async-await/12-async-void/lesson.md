...
order: 12
title: "async void Ã Â¢Ã¢â  Â¬ the Fire-and-Forget Trap"
...
Press **Run demo** Ã Â¢Ã¢â  Â¬ six fire-and-forget workers start, with no coordination. Three are `async Task` (their completion is tracked), three are `async void` (the demo has no way to know when they finish). Watch the timeline: the void workers appear, run, and vanish Ã Â¢Ã¢â  Â¬ untrackable.
...
> **Unit testing async void is impossible.** xUnit/NUnit cannot await void methods and cannot detect failures inside them. If your method is `async void` for testing reasons, refactor to `async Task` Ã Â¢Ã¢â  Â¬ test frameworks handle it.
...
- `async void` Ã Â¢Ã¢â ¬Â  only for event handlers. Everywhere else: `async Task`.
- Exceptions crash the process Ã Â¢Ã¢â  Â¬ always try/catch inside them.
