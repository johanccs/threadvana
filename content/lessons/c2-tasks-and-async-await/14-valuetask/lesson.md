...
order: 14
title: "ValueTask Ã Â¢Ã¢â  Â¬ Avoiding Allocations on Hot Paths"
difficulty: advanced
...
`Task` and `Task<T>` are reference types Ã Â¢Ã¢â  Â¬ every `await` that doesn't return synchronously allocates a Task on the heap. For millions of calls per second, that GC pressure adds up.

`ValueTask<T>` is a **struct** wrapper that can represent three states: a completed result (zero heap allocation), a real `Task<T>`, or a boxed `IValueTaskSource<T>` Ã Â¢Ã¢â  Â¬ an advanced low-level token. The first case is the key one: when a method has a cached result and returns immediately, `new ValueTask<T>(result)` costs nothing.

## The real-world picture

A pizza place that prepackages popular slices. If you order pepperoni (cached), the cashier hands you a slice from the warmer Ã Â¢Ã¢â  Â¬ no box (heap allocation) needed. If you order anchovy-pineapple (fresh), they make it from scratch and put it in a box (real Task).

## How it works in C#

```csharp
// High-frequency API Ã Â¢Ã¢â  Â¬ result is cached 90% of the time.
...
- Use `v.Preserve()` to escape Ã Â¢Ã¢â  Â¬ wraps the value in a `Task` that can be awaited multiple times.
- Never `.Result` or `.Wait()` a `ValueTask` Ã Â¢Ã¢â  Â¬ it may be already consumed.
...
- `ValueTask<T>` Ã Â¢Ã¢â ¬Â  struct, zero-allocation for synchronous results.
