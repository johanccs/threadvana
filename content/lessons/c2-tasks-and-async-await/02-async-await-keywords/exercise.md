...
`Solution.MakeCoffeeAsync()` currently uses `.Result` and `.Wait()` — it parks
...
- `BoilWaterAsync()` — takes a moment, then logs `"water boiled"` and returns `true`.
- `PourCoffee()` — logs `"coffee poured"`.

Rewrite `Solution.MakeCoffeeAsync()` so that it:

1. Has the signature `public static async Task<string> MakeCoffeeAsync()`.
2. `await`s `BoilWaterAsync()` first — pause the method, not a thread.
3. Then `await`s `PourCoffee()` — pouring must happen AFTER the water is boiled.
...
## Hints
1. Change the signature to `public static async Task<string> MakeCoffeeAsync()` — `async` is what unlocks `await` inside.
2. The pattern is: `await BoilWaterAsync();` then `await PourCoffee();` then `return "coffee ready";` — three lines, in that order.
