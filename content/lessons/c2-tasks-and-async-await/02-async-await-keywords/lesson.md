...
order: 2
title: async and await  -  Pause Here, Come Back Later
difficulty: beginner
...
- **`await`** marks the exact spot where it pauses: "wait for this task here —
...
Back to the coffee shop. You have your buzzer from *Meet the Task — a Promise of
a Future Result*. Standing at the counter staring at the barista until the drink
is done would be silly — you are stuck, and nobody else can be served.
...
off* — the very next step in your method.
...
- `Task` — the method finishes later, no result inside. Callers can still await it.
- `Task<T>` — the method finishes later *and* delivers a `T`. `await` unwraps it.
- `async void` — avoid it! Nobody gets a receipt, so nobody can await it, and a
...
`await` runs on one thread. During the wait, that thread is set free — look at
the grey gap. The line *after* the `await` may resume on a **different** pool
thread. Same method, new worker — the state machine doesn't care.

## Watch out

- You might think `await` blocks the thread. It does the opposite — it hands the
...
- You might call an async method and forget `await`. The work still starts, but
  your code rushes ahead without the result — like walking out before the
  buzzer rings.

## Key takeaways

- `async` = this method can pause; `await` = pause here, resume me later.
- During the pause, the thread is FREE — released, not blocked.
...
- If you reach for `.Result` or `.Wait()`, stop — you want `await`.
