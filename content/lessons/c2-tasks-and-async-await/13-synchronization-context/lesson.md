...
order: 13
title: "The SynchronizationContext  -  Where Continuations Go"
difficulty: advanced
...
interview:
  - q: "What is a SynchronizationContext?"
    a: "It is a scheduler that decides WHERE the code after an await runs  -  back on the original thread (UI, request context) or on a thread-pool thread. WPF and WinForms install one that posts back to the main UI thread. ASP.NET Framework installs one bound to the HTTP request. ASP.NET Core (8+) does NOT install one  -  continuations always run on the pool. Library code uses ConfigureAwait(false) to tell the awaiter: don't capture or post back, I am fine on any thread."
  - q: "How does ConfigureAwait(false) interact with SynchronizationContext?"
    a: "ConfigureAwait(false) tells the awaiter to NOT call SynchronizationContext.Post  -  the continuation is scheduled directly on the thread pool. As far as the continuation is concerned, there is no context. This avoids deadlocks in UI threads and request threads when library code is called from blocking callers."
---
...
When you `await`, the compiler captures the current `SynchronizationContext` and, after the awaited task finishes, **posts** the rest of your method back to that context — so you wake up on the same "kind" of thread you left.
...
// WPF app — DispatcherSynchronizationContext installed automatically.
// After await, the continuation magically re-enters the UI thread.
private async void Button_Click()
{
    await File.ReadAllTextAsync("data.txt");
    label.Text = "Loaded"; // safe — we are back on the UI thread
}
```

In your Sandbox (console), `SynchronizationContext.Current` is `null`. That means every continuation runs on the thread pool — there is no "home" to return to. This is why `.Result` doesn't deadlock in console apps or ASP.NET Core.

## Watch out

> **SynchronizationContext.Current can change mid-method.** Each `await` recaptures whatever context is current at that point — it is not cached once and reused.
...
- `SynchronizationContext` decides where code after `await` runs — UI thread, request thread, or pool.
- Console and ASP.NET Core → null → continuations on the pool.
- WPF and WinForms install a UI context; ASP.NET Framework installs a request context.
- `ConfigureAwait(false)` → skip the context, always run on the pool.
