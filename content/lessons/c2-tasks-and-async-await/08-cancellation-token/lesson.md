...
order: 8
title: CancellationToken Ã Â¢Ã¢â  Â¬ Cancelling Politely
...
interview:
  - q: What is the difference between CancellationToken and CancellationTokenSource?
    a: CancellationTokenSource is the "trigger" Ã Â¢Ã¢â  Â¬ you call .Cancel() on it. CancellationToken is the "listener" Ã Â¢Ã¢â  Â¬ you poll .IsCancellationRequested or pass it to async methods. You create the source, hand out its Token, and cancel the source when you want the work to stop. Dispose the source to free its timer resources.
...
A `CancellationToken` is a polite tap on the shoulder Ã Â¢Ã¢â  Â¬ it says "please stop what you are doing." It does NOT kill threads; it is a cooperative flag that methods check. If you ignore it, nothing happens (unlike `Thread.Abort`, which is deprecated for good reason).
...
A chef has a buzzer that the manager can press to say "stop this order." The chef finishes the current sautÃ Æ Ã Â© toss (they don't drop the pan mid-air), then stops. A `CancellationToken` is that buzzer Ã Â¢Ã¢â  Â¬ cooperative, not violent.
...
Press **Run demo** Ã Â¢Ã¢â  Â¬ we start a long operation with a 3-second CancellationToken. The timeline shows the work starting, the token being signalled at 3s, and the task catching the exception and finishing clean. No thread is aborted Ã Â¢Ã¢â  Â¬ the task simply stops when it checks the token.
...
> **Don't catch OperationCanceledException unless you mean to.** If you swallow it, the caller won't know the operation was cancelled Ã Â¢Ã¢â  Â¬ they will think it finished successfully.
...
- BCL methods throw `OperationCanceledException` when the token is signalled Ã Â¢Ã¢â  Â¬ catch it to perform cleanup, then re-throw.
