...
1. Start a slow, cancellable task by calling `Solution.SlowWorkAsync(CancellationToken token)` —
...
4. Always `Dispose()` the `CancellationTokenSource` — the best way is a `using` block.

## Hints
1. `new CancellationTokenSource(TimeSpan.FromMilliseconds(timeoutMs))` or `new CancellationTokenSource(timeoutMs)`.
2. `using var cts = ...;` Dispose is automatic — otherwise `try/finally { cts.Dispose(); }`.
3. `OperationCanceledException` — a using for `System` adds it; catch that specific exception.
