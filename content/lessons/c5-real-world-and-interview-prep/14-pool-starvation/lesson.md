...
order: 14
title: "Thread Pool Starvation  -  the Silent Slowdown"
difficulty: advanced
...
interview:
  - q: "What is thread pool starvation?"
    a: "When all pool threads are blocked (sleeping, waiting on locks, or doing sync I/O), new work queued to the pool (via Task.Run, Timer callbacks, async continuations) cannot start  -  it sits in the queue until a thread frees up. Symptoms: requests taking longer and longer under load, timeouts at the load balancer, but CPU is idle. The pool grows slowly (roughly 1 thread/sec) so recovery is sluggish."
