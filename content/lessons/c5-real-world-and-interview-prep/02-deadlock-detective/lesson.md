...
order: 2
title: Deadlock Detective  -  Spot the Circular Wait
difficulty: advanced
...
interview:
  - q: What causes a deadlock?
    a: "Four conditions must all be true  -  mutual exclusion (locks), hold-and-wait (thread holds lock A while waiting for B), no preemption (locks cannot be taken away), and circular wait (A waits for B, B waits for A). Break any one and the deadlock is impossible."
