...
Fix it by making Thread 2 lock A first, then B â   the same order as Thread 1.
One change is all it takes.

## Hints
1. Thread 2's locks should be `lock (LockA)` then inside it `lock (LockB)` â   the opposite of what it does now.
