...
interview:
  ...
  - q: "What if the service is read-heavy Ã Â¢Ã¢â  Â¬ 99% reads, 1% writes? Does your design change?"
    a: "ReaderWriterLockSlim for the mapping, or a cache-first architecture with a stale-reads TTL. The ConcurrentDictionary already handles reads without locking, so it's fine as-is for the lookup path. The write path is the bottleneck Ã Â¢Ã¢â  Â¬ batch DB writes on a background Channel."
---
