...
order: 4
title: Rate Limiter  -  the Token Bucket with SemaphoreSlim
...
interview:
  - q: Design a rate limiter that allows N requests per second. What synchronization primitive would you use?
    a: A token-bucket with SemaphoreSlim(N)  -  a timer releases one token every 1/N seconds. SemaphoreSlim.WaitAsync blocks incoming requests when the bucket is empty; the passing requests decrease the count. This is the answer most system-design interviews expect.
  - q: What happens when you use a simple lock + counter instead of a semaphore for rate limiting?
    a: A counter can't block  -  requesters must poll (spin-sleep) which wastes CPU and is never precise. SemaphoreSlim natively blocks until capacity is available, and WaitAsync gives you cooperative, non-blocking waiting when called from async code.
---
