...
interview:
  - q: "What happens when you call Release() on a SemaphoreSlim more times than the max count?"
    a: "SemaphoreFullException is thrown. The semaphore tracks current count  -  it cannot exceed the maxCount passed to the constructor. Unlike the OS Semaphore, which silently ignores extra releases, SemaphoreSlim enforces the upper bound."
