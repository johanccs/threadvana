...
order: 13
title: "Parallel vs Async  -  CPU-Bound vs I/O-Bound"
...
interview:
  - q: "How do you decide between Parallel.ForEach and Task.WhenAll with async?"
    a: "Answer this question: is each unit of work waiting on the CPU or on external I/O? CPU  -  Parallel.ForEach (keeps cores busy, synchronous lambdas, threads don't yield). I/O  -  Task.WhenAll or Parallel.ForEachAsync (threads yield while waiting, other work runs in the gap). Using Parallel.ForEach for I/O starves the pool by blocking threads on network/disk waits."
  - q: "Can you mix Parallel.ForEach with async inside?"
    a: "No  -  Parallel.ForEach expects an Action<T>, not a Func<T, Task>. Using async void or Task.Run inside it creates unobserved tasks and breaks the parallel barrier. Use Parallel.ForEachAsync instead."
---
