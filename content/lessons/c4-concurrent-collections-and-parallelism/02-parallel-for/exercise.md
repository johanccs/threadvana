...
around the loop). The harness checks ElapsedMs < 150 â   parallel should be faster.
...
3. The body calls `SlowSquare(i)` but that doesn't compute a result â   it just burns CPU. That's intentional.
