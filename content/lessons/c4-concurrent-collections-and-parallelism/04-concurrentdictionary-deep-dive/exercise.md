...
then verifies the totals are exactly right — no lost updates, no double-counts.
...
3. No `lock` needed — the exercise is about using the dictionary's built-in methods.
