...
order: 7
title: "ConcurrentBag Ã Â¢Ã¢â  Â¬ When Order Doesn't Matter"
difficulty: beginner
...
- Per-thread local storage Ã Â¢Ã¢â ¬Â  fast for add+take on the same thread.
- Steals from other threads when local is empty Ã Â¢Ã¢â  Â¬ best for work-stealing patterns.
- No FIFO/LIFO guarantee Ã Â¢Ã¢â  Â¬ use `ConcurrentQueue` when order matters.
