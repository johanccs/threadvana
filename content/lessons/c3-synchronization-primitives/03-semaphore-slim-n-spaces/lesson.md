...
order: 3
title: SemaphoreSlim Ã Â¢Ã¢â  Â¬ the Parking Lot with N Spaces
...
In *lock Ã Â¢Ã¢â  Â¬ One Key to the Bathroom* the key had exactly one copy. A semaphore
...
car drives in. The lot does not care WHO parks Ã Â¢Ã¢â  Â¬ only HOW MANY.
...
skipped, a space is lost FOREVER Ã Â¢Ã¢â  Â¬ until nobody gets in.

`SemaphoreSlim` vs `Semaphore`, in one line: Slim is the modern, lighter,
async-friendly one Ã Â¢Ã¢â  Â¬ reach for it by default.
...
- You might write `new SemaphoreSlim(1)`. That is a lock with extra steps Ã Â¢Ã¢â  Â¬ if
...
- Always Release in `finally` Ã Â¢Ã¢â  Â¬ a lost permit never comes back.
