...
`CallApiLimitedAsync()` just calls it directly — no limit at all — so when the
checker starts 6 calls at once, all 6 pile in together.

Your job — give the API a parking lot with 2 spaces:

1. Add the semaphore: `public static SemaphoreSlim Lot = new SemaphoreSlim(2);`
2. Rewrite `CallApiLimitedAsync()`:
   - `await Lot.WaitAsync();` — drive in (or queue at the entrance).
   - `try { await CallApiAsync(); }`
   - `finally { Lot.Release(); }` — ALWAYS drive out, even on a crash.

The checker starts 6 calls at once and watches: never more than 2 inside, all
6 finish, and NOT serialized to one-at-a-time either — that would be a lock,
not a semaphore!
