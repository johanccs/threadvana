...
order: 18
title: "ThreadLocal<T>  -  a Private Copy per Thread"
...
interview:
  - q: "What is ThreadLocal<T> and when would you use it?"
    a: "It gives each thread its own private copy of a value  -  no locking needed because no thread touches another thread's copy. Classic use: a per-thread Random instance (Random is not thread-safe). Create it with a factory: new ThreadLocal<Random>(() => new Random()), then access .Value from any thread  -  each gets its own instance. The per-thread data is isolated, so there are no race conditions."
  - q: "What's the difference between [ThreadStatic] and ThreadLocal<T>?"
    a: "[ThreadStatic] is an attribute on a static field  -  each thread gets its own copy, but there is NO initialisation per thread (it is null/default each time). ThreadLocal<T> accepts a factory delegate that runs once per thread to seed the value. ThreadLocal<T> is the modern, safer choice."
---
