...
order: 7
title: "lock Best Practices Ã Â¢Ã¢â  Â¬ What to Lock On"
difficulty: beginner
...
`lock` is the simplest synchronisation primitive in C# Ã Â¢Ã¢â  Â¬ one thread enters, others wait. But three details decide whether your lock is safe or a time bomb: what you lock ON, what you do INSIDE the lock, and how long you hold it.
...
| **Reference type** | Any `object` | Value types (box silently Ã Â¢Ã¢â ¬Â  no lock) |
| **Readonly** | `readonly` field | Reassignable Ã Â¢Ã¢â  Â¬ lock silently switches object |
...
public void Transfer(Account from, Account to, decimal amount)
{
    ...
    // DANGEROUS: await inside lock Ã Â¢Ã¢â  Â¬ the thread may change after await.
...
> **Never `await` inside a `lock`.** The thread that enters the lock may not be the same thread that returns from `await` Ã Â¢Ã¢â  Â¬ the lock will not be released on the original thread, causing a violation.
...
> **Don't lock on interned strings.** `"MyLock"` is shared across the process Ã Â¢Ã¢â  Â¬ multiple classes locking on the same string will accidentally serialise with each other.
