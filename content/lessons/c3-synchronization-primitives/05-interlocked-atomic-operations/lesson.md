...
order: 5
title: Interlocked Ã Â¢Ã¢â  Â¬ Atomic Operations Without a Lock
difficulty: beginner
...
indivisible Ã Â¢Ã¢â  Â¬ the read-add-write from *Race Conditions Up Close* becomes ONE
step that no thread can interrupt.

It lives in `System.Threading` and works on `int` and `long` fields. No lock
object, no queuing threads Ã Â¢Ã¢â  Â¬ the CPU does the guarding.
...
write" sequence for anyone to interrupt Ã Â¢Ã¢â  Â¬ the step has no middle.
...
Press **Run demo**. Two pairs of workers do the same hammering as last lesson Ã Â¢Ã¢â  Â¬
...
- You might protect one variable with Interlocked and leave a second one
  plain. Interlocked guards exactly ONE variable per call Ã Â¢Ã¢â  Â¬ rules spanning
...
- You might read the field plainly while others write it. A plain read can
  see a stale value Ã Â¢Ã¢â  Â¬ use `Volatile.Read` for a fresh snapshot.

## Key takeaways

- Interlocked makes read-add-write one indivisible CPU step Ã Â¢Ã¢â  Â¬ no lock needed.
- Use it for single-number updates: `Increment`, `Add`, `Exchange`.
- `CompareExchange` = "swap only if unchanged" Ã Â¢Ã¢â  Â¬ the lock-free building block.
