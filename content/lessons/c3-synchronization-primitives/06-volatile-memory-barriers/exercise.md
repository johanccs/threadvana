...
## Hints
1. `while (!IsRunning) { }` — but this won't work without volatile or explicit Volatile.Read.
2. Since `IsRunning` is already `volatile`, the simple `while` loop works.
3. The challenge is understanding WHY volatile is needed here — the starter has it, your code just needs the loop.
