...
order: 9
title: "Deadlocks and Lock Ordering Ã Â¢Ã¢â  Â¬ Prevention by Convention"
difficulty: advanced
...
interview:
  - q: "What causes a deadlock with locks?"
    a: "Two or more threads each hold a lock the other needs Ã Â¢Ã¢â  Â¬ a circular wait. Thread A locks X then tries Y; Thread B locks Y then tries X. Both wait forever. The fix is a consistent lock ORDERING: both threads must always acquire X first, then Y. If the order must differ, acquire both under one larger-scope lock, or use Monitor.TryEnter with a timeout as a back-out mechanism."
  - q: "How do you detect a deadlock in production?"
    a: "dotnet-dump or PerfView can capture the thread stacks of a live process Ã Â¢Ã¢â  Â¬ you'll see threads waiting on Monitor.Enter or WaitOne, and you can match lock addresses to find the circular dependency. Visual Studio's Parallel Stacks window visualises this. The best defence is consistent lock ordering + keeping the number of locks small."
---

## What is it?

A deadlock is the ultimate circular dependency: thread A holds lock #1 and wants lock #2; thread B holds lock #2 and wants lock #1. Neither can progress. The process freezes Ã Â¢Ã¢â  Â¬ no exceptions, no crashes, just a silent hang.
...
// Thread B Ã Â¢Ã¢â  Â¬ opposite order!
...
```
Rule 1: Keep lock count small Ã Â¢Ã¢â  Â¬ every extra lock multiplies deadlock risk.
...
Press **Run demo** Ã Â¢Ã¢â  Â¬ two threads lock in opposite orders. The timeline shows both entering their first lock, then both stuck forever waiting for the second. After 2 seconds the sandbox timeouts and reports the hang.

## Watch out

> **The Thread.Join deadlock.** If thread A calls threadB.Join() while threadB calls threadA.Join(), same deadlock Ã Â¢Ã¢â  Â¬ just using threads instead of locks. Always Join in a known, consistent order.

> **SemaphoreSlim can deadlock too.** If thread A holds a semaphore slot and then awaits another semaphore that thread B holds Ã Â¢Ã¢â  Â¬ while B is waiting on A's semaphore Ã Â¢Ã¢â  Â¬ same circular wait. Same prevention: consistent ordering.
