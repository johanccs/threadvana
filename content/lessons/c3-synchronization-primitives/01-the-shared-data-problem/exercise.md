...
written out as its three real steps — READ, ADD, WRITE — and right now NOTHING
...
Both threads run the same method, so both will automatically use the SAME key —
...
## Hints
1. The three lines `int temp = Counter; temp = temp + 1; Counter = temp;` must ALL sit inside the same lock — together they become one unbreakable step.
2. The syntax is: `lock (Solution.Gate) { ... }` — put the three lines between the braces.
3. Leave `Thread.Yield()` alone — it is only there to make the race visible, and the lock still wins.