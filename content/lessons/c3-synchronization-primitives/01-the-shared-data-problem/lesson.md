...
order: 1
title: The Shared Data Problem Ã Â¢Ã¢â  Â¬ When Workers Trip Over Each Other
difficulty: beginner
...
When two threads touch the same data at the same time, they can corrupt it Ã Â¢Ã¢â  Â¬
...
Anna comes back and writes "4" as well Ã Â¢Ã¢â  Â¬ she still had "3" in her head.
...
The fix is a lock Ã Â¢Ã¢â  Â¬ think of a single bathroom key; only one person inside at
a time:
...
Both threads must lock on the SAME object Ã Â¢Ã¢â  Â¬ two different keys open two
different bathrooms.
...
- You might think "it worked on my machine". Races hide: they need unlucky
  timing, so they pass 99 times and fail once Ã Â¢Ã¢â  Â¬ usually in production.
...
- `counter++` is read + add + write Ã Â¢Ã¢â  Â¬ never one step.
