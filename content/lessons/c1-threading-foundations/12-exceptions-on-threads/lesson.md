...
thread has its own call stack. An error on Thread A does not interrupt Thread B —
unless you design a way to pass it over.

If nobody catches the exception, the worker dies. In older .NET the rest of the
program kept running (which was worse — it just stopped working). In modern .NET
the whole process shuts down.

## The real-world picture

Two cooks are prepping vegetables. One cuts herself and leaves the kitchen. The
other cook keeps chopping — she doesn't automatically know about the accident.
...
other cook's shoulder the whole time — that's try/catch inside the thread.
...
new Thread(() =>
{
    ...
    {
        // The error stays on THIS thread — handle it here.
...
exception travels back to whoever called `await` — no try/catch in the background
thread needed!

## See it move

Press **Run demo**. A background task throws — watch it die in its swimlane. The
main lane keeps going. Then watch the fixed version catch and report it.

## Watch out

- You might expect the main thread to crash when a worker throws. It doesn't — but
