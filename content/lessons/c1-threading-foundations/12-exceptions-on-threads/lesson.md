...
interview:
  - q: What happens if an exception is thrown on a worker thread and nobody catches it?
    a: The worker thread dies and, in modern .NET, the whole process usually ends. Exceptions do not magically appear on the main thread Ã¢â‚¬â€ you must catch them inside the thread's own code.
