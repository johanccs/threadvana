...
3. After the worker finishes (use a short delay or wait), check that
   `Solution.SeenByMain` is still `"all clear"` — the main thread was unaffected.
...
## Hints
1. Wrap the thread-pool work inside `new Thread(() => { ... }).Start()` — the DEMO
   shows the pattern.
...
3. Track the main thread's view: after the thread finishes, check
   `SeenByMain` — but do NOT set it inside the worker. The starter already sets it
