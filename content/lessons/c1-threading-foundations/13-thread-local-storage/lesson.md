...
order: 13
title: Thread-Local Storage Ã Â¢Ã¢â  Â¬ Every Thread Gets Its Own Copy
...
interview:
  ...
  - q: What is the difference between [ThreadStatic] and ThreadLocal&lt;T&gt;?
    a: "The [ThreadStatic] attribute is simpler but cannot set an automatic default Ã Â¢Ã¢â  Â¬ every thread starts at null/0. ThreadLocal<T> lets you provide a factory so every thread begins with a proper value."
---

## What is it?

Normally a `static` field in C# is **shared** Ã Â¢Ã¢â  Â¬ every thread sees the same value.
...
clipboard Ã Â¢Ã¢â  Â¬ Worker B cannot read it, and vice versa.

## The real-world picture

A restaurant has one whiteboard for all waiters (static field). Everyone writes
their own table number on it Ã Â¢Ã¢â  Â¬ chaos!
...
{
    _count++;
    Console.WriteLine(_count); // prints 1 (not 2 Ã Â¢Ã¢â  Â¬ different copy!)
}).Start();
```
...
- It is perfect for cached objects or per-thread counters Ã Â¢Ã¢â  Â¬ no locks needed.
- `ThreadLocal<T>` is the modern, initial-value-friendly alternative.
- Neither `[ThreadStatic]` nor `ThreadLocal` works with async/await Ã Â¢Ã¢â  Â¬ see Category 2.
