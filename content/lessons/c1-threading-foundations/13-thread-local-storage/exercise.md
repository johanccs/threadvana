...
The starter has a shared `static int _counter` that both threads fight over — they
...
independent count (each increments once → each sees 1, not 2).

## Hints
1. Add `[ThreadStatic]` before the `public static int Counter` field. That's it.
2. With `[ThreadStatic]` the default value is 0 for every thread — which is exactly
   what a counter starts at. No initialiser needed.
3. The starter code is the same as the solution — you literally just add
