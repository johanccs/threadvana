...
order: 11
title: Tuning the Thread Pool Ã Â¢Ã¢â  Â¬ Min Threads and the Slow Ramp-Up
difficulty: advanced
...
interview:
  - q: What does SetMinThreads do and when would you use it?
    a: It tells the pool to keep a minimum number of workers ready at all times. You use it when bursts of work arrive all at once and you do not want to wait for the pool's slow ramp-up Ã Â¢Ã¢â  Â¬ one new worker can take about 500 ms to appear.
  - q: Should you ever change the maximum thread count?
    a: Rarely. The default is huge (thousands). Raising it can hide a real problem Ã Â¢Ã¢â  Â¬ if you are running out of threads, more threads usually make things slower, not faster. Fix the bottleneck instead.
---

## What is it?

The thread pool does not create all its workers at once. When many tasks arrive at the
same time, the pool adds workers *slowly* Ã Â¢Ã¢â  Â¬ roughly one every half-second. This is
...
and ready Ã Â¢Ã¢â  Â¬ don't wait for the burst to ramp you up."

## The real-world picture

A restaurant with one waiter can handle a steady stream of two tables. But a bus of
tourists just pulled up outside Ã Â¢Ã¢â  Â¬ 40 people need to order NOW. The restaurant
very slowly sends in one extra waiter every 30 seconds while the tourists grumble.

`SetMinThreads` is the manager saying "I knew the bus was coming Ã Â¢Ã¢â  Â¬ I kept 10 waiters
on shift."
...
- `SetMinThreads` does **not** reserve actual threads Ã Â¢Ã¢â  Â¬ it only means "grow quickly
...
min threads Ã Â¢Ã¢â  Â¬ watch the queue drain slowly. Phase 2 runs with `SetMinThreads(12,12)`
Ã Â¢Ã¢â  Â¬ the queue empties fast.

## Watch out

- You might think `SetMinThreads` instantly creates threads. It doesn't Ã Â¢Ã¢â  Â¬ it just
...
- `SetMinThreads` removes the throttle Ã Â¢Ã¢â  Â¬ tell it how many workers you will need.
