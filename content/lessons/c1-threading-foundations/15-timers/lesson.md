...
order: 15
title: Timers Ã Â¢Ã¢â  Â¬ Running Code on a Schedule
...
- Timer callbacks should be FAST. If one takes 2 seconds, the next scheduled tick
  may overlap Ã Â¢Ã¢â  Â¬ use a flag or lock to prevent re-entry.
- `System.Timers.Timer` (with an `s`) is an older wrapper Ã Â¢Ã¢â  Â¬ just use
