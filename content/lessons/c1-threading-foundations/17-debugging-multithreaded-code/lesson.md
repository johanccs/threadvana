...
order: 17
title: Debugging Multithreaded Code Ã Â¢Ã¢â  Â¬ Survival Tactics
difficulty: advanced
...
interview:
  ...
  - q: Why are multithreaded bugs harder to fix than single-threaded ones?
    a: Because they are non-deterministic Ã Â¢Ã¢â  Â¬ they depend on the exact timing of the OS scheduler. The same code can work 49 times and fail on the 50th. This makes reproducing the bug the first and hardest step.
---

## What is it?

Single-threaded bugs happen the same way every time. Multithreaded bugs play
hide-and-seek Ã Â¢Ã¢â  Â¬ they might only show up 2% of runs because they depend on the
...
2. **Log ruthlessly** Ã Â¢Ã¢â  Â¬ timestamps, thread id, what the thread is about to do.
3. **Reproduce under stress** Ã Â¢Ã¢â  Â¬ run the buggy section 100 times in a loop.
...
"Thread 7, Thread 8, Thread 11" Ã Â¢Ã¢â  Â¬ debugging blind.
...
- Don't rely ONLY on debugger breakpoints Ã Â¢Ã¢â  Â¬ they freeze time and hide races.

## Key takeaways

- Name every thread Ã Â¢Ã¢â  Â¬ it's a one-liner that saves hours of debugging.
