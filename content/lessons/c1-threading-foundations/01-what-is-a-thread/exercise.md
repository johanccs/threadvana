...
*different* id than the thread that called `Run()` — proof that a new worker really
did the job.

## Hints
1. `new Thread(() => { ... })` takes a lambda — the code between the braces is what the new worker runs.
