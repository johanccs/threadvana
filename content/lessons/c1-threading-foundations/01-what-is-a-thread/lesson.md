...
interview:
  ...
  - q: What is the difference between a process and a thread?
    a: A process is your running program with its own memory. Threads are the workers inside that process Ã Â¢Ã¢â  Â¬ they share the same memory, which is exactly why two threads can accidentally mess with the same data.
---
...
be inside your program *at the same moment* Ã Â¢Ã¢â  Â¬ that is the whole point, and also the
...
- `Start()` sets it running. The order in which the two threads' lines run is
  **not guaranteed** Ã Â¢Ã¢â  Â¬ run the demo twice and compare!
...
thread and the worker thread overlap in time Ã Â¢Ã¢â  Â¬ both are running at once. Then look at
...
- Threads share the same memory Ã Â¢Ã¢â  Â¬ great for teamwork, dangerous for shared data.
