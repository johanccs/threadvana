...
order: 14
title: Telling a Thread to Stop Ã Â¢Ã¢â  Â¬ Cooperative Cancellation
...
interview:
  - q: How do you stop a running thread?
    a: "You do not stop a thread by force (Thread.Abort does not exist on .NET Core). Instead ask it to cooperate Ã Â¢Ã¢â  Â¬ a bool flag the worker checks regularly. In modern .NET, CancellationToken is the standard."
...
writing to a file, or updating shared data Ã Â¢Ã¢â  Â¬ killing it could leave everything broken.
...
// Ã Â¢Ã¢â  Â¬Ã Â¦ later, from another thread:
_shouldStop = true;
worker.Join(); // wait for the clean exit
```

The `volatile` keyword makes sure the worker actually SEES the new value Ã Â¢Ã¢â  Â¬ without
...
- .NET Core has no `Thread.Abort`. That's a good thing Ã Â¢Ã¢â  Â¬ cooperative cancellation
  is the only correct way. Use it.

## Key takeaways

- Never kill a thread Ã Â¢Ã¢â  Â¬ ask it to stop politely.
